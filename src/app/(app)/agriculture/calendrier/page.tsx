...
export default function CulturalCalendarPage() {
  ...
  const [crop, setCrop] = useState<string>(CROP_IDS[0]);
  ...
  const cropLabel = (cropId: string) => {
    const key = cropLabelKey(cropId);
    const translated = t(key);
    return translated === key ? cropId : translated;
  };
  ...
  const addPreset = (taskKey: string, offsetDays: number) => {
    const d = new Date();
    ...
    <div className="min-h-screen bg-[#F5F5F0] pb-24">
      ...
      <main className="mx-auto max-w-lg space-y-4 p-4">
        <section className="rounded-2xl bg-white p-4 shadow-sm">
          ...
          {upcoming.length === 0 ? (
            <p className="mt-2 text-xs text-gray-500">{t("calendar.noUpcoming")}</p>
          ) : (
            <ul className="mt-2 space-y-2">
              {upcoming.map((item) => (
                <li key={item.id} className="flex items-center justify-between rounded-lg bg-emerald-50 p-3 text-xs">
                  <div>
                    <p className="font-medium">
                      {item.dueDate} — {cropLabel(item.crop)}: {item.task}
                    </p>
                  ...
                  <Button
                    ...
                    size="sm"
                    variant="outline"
                    onClick={() => setTasks(toggleCulturalTask(item.id, storeId))}
                  >
        ...
        <form onSubmit={submit} className="space-y-3 rounded-2xl bg-white p-4 shadow-sm">
          <h2 className="text-sm font-semibold">{t("calendar.addTask")}</h2>
          <div>
            <Label>{t("calendar.crop")}</Label>
            <select
        ...
        <section className="rounded-2xl bg-white p-4 shadow-sm">
          <h2 className="mb-2 text-sm font-semibold">
            {t("calendar.presets")} — {cropLabel(crop)}
          </h2>
          <div className="flex flex-wrap gap-2">
            {presets.map((preset) => (
              <Button
                ...
                size="sm"
                variant="outline"
                onClick={() => addPreset(preset.key, preset.offsetDays)}
              >
                {t(preset.key)}
              </Button>
            ))}
