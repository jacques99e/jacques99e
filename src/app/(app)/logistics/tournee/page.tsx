...
export default function TourneePage() {
  ...
  const shareRoute = () => {
    ...
    const msg = `🚚 Tournée du jour (${tour.length} colis)\n\n${lines.join("\n\n")}`;
    void navigator.clipboard.writeText(msg);
    window.open(buildWhatsAppShareUrl(msg), "_blank", "noopener,noreferrer");
  };

  const shareOne = (d: Delivery) => {
    const msg = `Colis pour ${d.recipient_name} : ${trackingUrl(d.tracking_code)}`;
    ...
    <div className="min-h-screen bg-[#F5F5F0] pb-24">
      <header className="bg-sky-700 px-4 py-4 text-white shadow-sm">
        <div className="mx-auto flex max-w-lg items-center justify-between">
          <h1 className="text-lg font-semibold">Tournée du jour</h1>
              ...
              <li key={d.id} className="rounded-2xl bg-white p-4 shadow-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-xs text-sky-600">Code {d.tracking_code}</p>
                  </div>
                </div>
                <div className="mt-2 flex gap-2">
                  <Button type="button" size="sm" variant="outline" className="flex-1" onClick={() => shareOne(d)}>
                    <MessageCircle className="mr-1 h-3 w-3" /> Client
                  </Button>
                  <Button asChild size="sm" variant="outline" className="flex-1">
                    <Link href={`/logistics/deliveries/${d.id}`}>Détail</Link>
                  </Button>
                </div>
              </li>
            ))}
