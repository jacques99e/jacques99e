...
export default function CreditLedgerPage() {
  ...
  const remind = (name: string, phone: string, balance: number) => {
    const msg = `Bonjour ${name}, votre solde chez nous est de ${formatCurrency(balance)}. Merci de régulariser quand possible.`;
    if (phone) {
    ...
    <div className="min-h-screen bg-[#F5F5F0] pb-24">
      <header className="bg-[#075E54] px-4 py-4 text-white shadow-sm">
        <div className="mx-auto flex max-w-lg items-center justify-between">
          <h1 className="text-lg font-semibold">Carnet crédit clients</h1>
      ...
      <main className="mx-auto max-w-lg space-y-4 p-4">
        <div className="rounded-2xl bg-white p-4 shadow-sm">
          ...
          <p className="mt-1 text-xs text-gray-500">{debtors.length} client(s) avec solde</p>
        </div>

        <form onSubmit={submit} className="space-y-3 rounded-2xl bg-white p-4 shadow-sm">
          <h2 className="text-sm font-semibold text-gray-800">Nouvelle écriture</h2>
          <div>
          ...
          <div>
            <Label>Note</Label>
            <Input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Ex: 2 sacs de riz" />
          </div>
          <Button type="submit" className="w-full">
            <Plus className="mr-1 h-4 w-4" /> Enregistrer
          </Button>
        </form>

        {debtors.length > 0 ? (
          <section className="rounded-2xl bg-white p-4 shadow-sm">
            <h2 className="mb-2 text-sm font-semibold">Clients à relancer</h2>
        ...
        <section className="rounded-2xl bg-white p-4 shadow-sm">
          <h2 className="mb-2 text-sm font-semibold">Historique récent</h2>
          {entries.length === 0 ? (
            <p className="text-xs text-gray-500">Aucune écriture pour le moment.</p>
          ) : (
            <ul className="space-y-2">
