    ...
    <div className="min-h-screen bg-[#F5F5F0] pb-24">
      <header className="bg-indigo-700 px-4 py-4 text-white shadow-sm">
        <div className="mx-auto flex max-w-lg items-center justify-between">
          <h1 className="text-lg font-semibold">{t("qr.title")}</h1>
      ...
      <main className="mx-auto max-w-lg space-y-4 p-4">
        <section className="rounded-2xl bg-white p-4 shadow-sm">
          <div className="mb-3 flex items-center gap-2 text-indigo-800">
            ...
            <p className="text-sm text-gray-600">{t("qr.printHint")}</p>
          </div>
          <div>
            <Label>{t("qr.assetLabel")}</Label>
            <select
              className="mt-1 w-full rounded-lg border px-3 py-2 text-sm"
              value={selectedId}
              onChange={(e) => setSelectedId(e.target.value)}
            >
              <option value="">{t("qr.chooseAsset")}</option>
              {assets.map((a) => (
        ...
        {selected && qrDataUrl ? (
          <section className="flex flex-col items-center rounded-2xl bg-white p-6 shadow-sm">
            ...
            <Image src={qrDataUrl} alt={t("qr.title")} width={220} height={220} unoptimized />
            ...
            <Button type="button" className="mt-4 w-full" onClick={copyLink}>
              ...
              {copied ? t("qr.linkCopied") : t("qr.copyVerifyLink")}
            </Button>
          </section>
        ) : (
          <p className="rounded-2xl bg-white p-4 text-center text-sm text-gray-500 shadow-sm">
            {assets.length === 0 ? (
              ...
              t("qr.selectToGenerate")
            )}
          </p>
        )}

        <section className="rounded-2xl border border-indigo-100 bg-indigo-50 p-4 text-xs text-indigo-900">
          <p className="font-semibold">{t("qr.exportTip")}</p>
          <p className="mt-1">{t("qr.exportTipDesc")}</p>
        </section>
      </main>
    </div>
  );
