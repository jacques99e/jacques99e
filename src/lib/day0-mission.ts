 ...
 * 1 produit → envoyer le lien MoMo. La caisse n'est plus le goulot.
 */

export type Day0StepId = "product" | "share";
...
export const DAY0_STEPS: Array<{
  ...
  {
    id: "share",
    title: "Envoyez le lien MoMo",
    description: "Le client paie tout seul. Vous n’ouvrez rien.",
    href: "/products?share=1&first=1",
    cta: "Envoyer le lien paiement",
  },
];
...
export function getDay0Step(
  ...
  if (productsCount >= 1 && (salesCount >= 1 || isDay0ShareDone())) return null;
...
export function isDay0MissionComplete(
  ...
  return productsCount >= 1 && (salesCount >= 1 || isDay0ShareDone());
}
...
export function getDay0Progress(productsCount: number, salesCount: number) {
  ...
  const done = [productDone, shareDone].filter(Boolean).length;
  return {
    productDone,
    saleDone,
    shareDone,
